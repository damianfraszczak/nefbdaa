...
namespace NEFBDAACommons.Shared.DynamicQuery
{
    ...
    public static class QueryBuilder
    {
        ...
        public static IQueryable<T> BuildQuery<T>(this IEnumerable<T> queryable, FilterRule filterRule,
            bool useIndexedProperty = false, string indexedPropertyName = null)
        {
            string parsedQuery;
            return BuildQuery(queryable.AsQueryable(), filterRule, out parsedQuery, useIndexedProperty,
                indexedPropertyName);
        }
        ...
        /// <returns>Filtered IQueryable.</returns>
        public static IQueryable<T> BuildQuery<T>(this IQueryable<T> queryable, FilterRule filterRule,
            out string parsedQuery, bool useIndexedProperty = false, string indexedPropertyName = null)
        {
        ...
        /// <returns>Filtered IQueryable.</returns>
        public static IQueryable<T> BuildQuery<T>(this IQueryable<T> queryable, FilterRule filterRule,
            BuildExpressionOptions options, out string parsedQuery)
        {
        ...
        public static Expression<Func<T, bool>> BuildExpressionLambda<T>(this FilterRule filterRule,
            BuildExpressionOptions options, out string parsedQuery)
        {
            if (filterRule == null)
            {
            ...
            BuildExpressionOptions options)
        {
            ...
            if (rule.Field != null)
            {
                Type type = QueryBuilderHelper.GetTypeForStringType(rule.Type);
                    ...
                    else
                    {
                        ...
                        if (rule.Type == "date")
                        {
                            try
                            {
                                ...
                                propertyExp = Expression.Property(propertyExp, type.GetProperty("Date"));
                            }
                            catch (Exception)
                            {
                                try
                                {
                                    propertyExp = Expression.Property(propertyExp, type.GetProperty("Value"));
                                    propertyExp = Expression.Property(propertyExp, type.GetProperty("Date"));
                                }
                                catch (Exception ex)
                                {
                                    ApplicationLogger.LogError(typeof(QueryBuilder),
                                        $"Error with filtering by date {ReflectionHelper.GetNullableTypeIfIs(type)}",
                                        ex);
                                }
                            }
                        }
        ...
        private static Expression BuildNestedExpression(Expression expression,
            IEnumerator<string> propertyCollectionEnumerator, FilterRule rule, BuildExpressionOptions options,
            Type type)
        {
            while (propertyCollectionEnumerator.MoveNext())
            {
                var propertyName = propertyCollectionEnumerator.Current;
                var property = expression.Type.GetProperties()
                    .FirstOrDefault(x => x.Name.EqualsIgnoreCase(propertyName));
        ...
        private static Expression BuildOperatorExpression(Expression propertyExp, FilterRule rule,
            ...
            BuildExpressionOptions options)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    return new List<ConstantExpression>
                    {
                        Expression.Constant(value is DateTime
                            ? value
                            : tc.ConvertFromString(null, options.CultureInfo, value.ToString().Trim()))
                    };
                }
            }
        }
            ...
            BuildExpressionOptions options)
        {
            ...
            Expression exOut =
                Expression.GreaterThanOrEqual(propertyExp, Expression.Convert(someValue, propertyExp.Type));
